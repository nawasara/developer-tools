<div x-data="{ open: @entangle('isOpen') }" x-cloak>
    ...
    <button @click="open = true"
        class="fixed bottom-6 right-6 w-14 h-14 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center justify-center z-50 transition-all duration-300">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                @foreach ($commands as $cmd)
                    <button wire:click="{{ $cmd['method'] }}" wire:loading.attr="disabled"
                        class="flex items-center justify-center gap-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 text-xs py-2 px-3 rounded-lg hover:bg-blue-600 hover:text-white transition disabled:opacity-50">
